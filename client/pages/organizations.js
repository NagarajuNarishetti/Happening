...
export default function OrganizationsPage({ keycloak }) {
  ...
  // Auto-open overview modal when navigated with ?openOrgId=ID
  useEffect(() => {
    const openId = router?.query?.openOrgId;
    if (!openId || organizations.length === 0) return;
    const org = organizations.find(o => String(o.id) === String(openId));
    if (org) {
      handleViewDetails(org);
    }
  }, [router?.query?.openOrgId, organizations]);
        ...
        {organizations.length === 0 ? (
          ...
          <div className="space-y-10">
                  ...
                  {organizations.filter(o => (String(o.role).toLowerCase() === 'orgadmin' || String(o.role).toLowerCase() === 'owner')).map((org) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        {/* Hide Switch for OrgAdmin entries */}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {(organizations.filter(o => {
              ...
              return r === 'agent' || r === 'reviewer' || r === 'organizer';
            })).length > 0 && (
                <>
                  <div className="flex items-center justify-between mb-2">
            ...
            {(organizations.filter(o => {
              const r = String(o.role).toLowerCase();
              return r === 'customer' || r === 'viewer' || r === 'user';
            })).length > 0 && (
                <>
                  <div className="flex items-center justify-between mb-2">
